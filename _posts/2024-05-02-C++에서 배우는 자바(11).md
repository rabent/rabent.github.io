...
f.print("me",1);
{% endhighlight %}  
이러한 MyFunc 인터페이스를 구현한 익명 객체 대신 람다식을 사용할 수 있는 것은 람다식도 실제로는 익명 객체이고, 익명 객체의 메서드 max와 람다식의 매개변수 타입과 개수, 반환값이 일치하기 때문이다.  
...
최종 연산의 결과를 Optional 객체에 담아서 반환하는 이유는 간단하다. **null 체크**가 편하기 때문이다. if문을 사용하여 null 체크를 하지 않아도 Exception의 걱정 없이 코드를 사용할 수 있는 것이다.  
...
{% endhighlight %}  
위와 같이 사용하면 레벨 별로 학생들을 집계할 수 있다.  
